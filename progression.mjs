...
const fusionNames={
  turtle_bird:{name:'玄羽鼋',passive:'team_heal',mutation:'wing',effects:{health:42,haste:.08},text:'生命 +42，冷却 -8%，主动偏向团队治疗'},
  turtle_wolf:{name:'吼甲狰',passive:'taunt',mutation:'scale',effects:{health:58,taunt:1},text:'生命 +58，获得嘲讽，替后排吸火'},
  bird_wolf:{name:'雷牙鸮',passive:'chain',mutation:'horn',effects:{power:.12,haste:.05},text:'伤害 +12%，冷却 -5%，技能弹射'},
  bird_turtle:{name:'玄羽鼋',passive:'team_heal',mutation:'wing',effects:{health:42,haste:.08},text:'生命 +42，冷却 -8%，主动偏向团队治疗'},
  wolf_turtle:{name:'吼甲狰',passive:'taunt',mutation:'scale',effects:{health:58,taunt:1},text:'生命 +58，获得嘲讽，替后排吸火'},
  wolf_bird:{name:'雷牙鸮',passive:'chain',mutation:'horn',effects:{power:.12,haste:.05},text:'伤害 +12%，冷却 -5%，技能弹射'},
  all:{name:'混沌灵',passive:'rally',mutation:'ember',effects:{health:65,power:.14,haste:.06},text:'生命 +65，伤害 +14%，冷却 -6%，鼓舞全队'},
  turtle:{name:'山甲王',passive:'thorns',mutation:'scale',effects:{health:70,taunt:1},text:'生命 +70，获得嘲讽和反刺'},
  bird:{name:'九霄羽',passive:'sniper',mutation:'wing',effects:{power:.18,haste:.08},text:'伤害 +18%，冷却 -8%，远程狙击'},
  wolf:{name:'赤牙王',passive:'leech',mutation:'horn',effects:{power:.16,health:28},text:'生命 +28，伤害 +16%，攻击吸血'}
};
...
function fusionRecipe(s,materials){
  const primary=materials[0],cards=materials.map(u=>cardOf(s,u.cardId)||CARDS[u.cardId]).filter(Boolean),kindsUnique=[...new Set(cards.map(c=>c.kind))],combo=kindsUnique.length>=3?'all':kindsUnique.length===1?kindsUnique[0]:`${kindsUnique[0]}_${kindsUnique[1]}`;
  const base=fusionNames[combo]||fusionNames[primary.kind]||fusionNames.turtle;
  const count=materials.length,starSum=materials.reduce((sum,u)=>sum+u.stars,0),race=topKey(countBy(cards,c=>c.race),cardOf(s,primary.cardId)?.race||'spirit'),element=topKey(countBy(cards,c=>c.element),cardOf(s,primary.cardId)?.element||'spirit'),effects={...base.effects};
  effects.health=(effects.health||0)+(count===3?22:0)+(starSum- count)*16;
  if(count===3)effects.power=(effects.power||0)+.04;
  const names=cards.map(c=>c.name).join('＋');
  return {primary,name:base.name,kind:primary.kind,race,element,mutation:base.mutation,passive:base.passive,effects,text:base.text,source:names,cost:count===3?5:4};
}
export function fusionPreview(s,unitIds=[]){
  const unique=[...new Set(unitIds)],units=roster(s).filter(u=>unique.includes(u.id));
  if(s.phase!=='shop'||unique.length<2||unique.length>3||units.length!==unique.length)return null;
  const r=fusionRecipe(s,units),stars=Math.min(3,Math.max(...units.map(u=>u.stars))),card={id:'mut_preview',name:r.name,kind:r.kind,race:r.race,element:r.element,mutation:r.mutation,passive:r.passive,fusion:{name:'融合',text:r.text,effects:r.effects},rarity:'rare',cost:r.cost,text:`融合异兽 · ${r.source}`,hue:120};
  const stats=combatStats({...(s||{}),mutants:[...(s.mutants||[]),card]},card.id,stars,s.level,{});
  return {...card,stars,stats,materials:units,source:r.source};
}
export function fuseUnits(s,unitIds=[]){
  ...
  const hue=Math.floor(random(n)*300),card={id,name:recipe.name,kind:recipe.kind,race:recipe.race,element:recipe.element,mutation:recipe.mutation,passive:recipe.passive,fusion:{name:'融合',text:recipe.text,effects:recipe.effects},rarity:'rare',cost:recipe.cost,text:`融合异兽 · ${recipe.source}。${recipe.text}`,hue};
  n.mutants=[...(n.mutants||[]),card];n.owned[id]=1;
  ...
  n.message=`${materials.map(u=>cardName(s,u.cardId)).join('＋')}融合成${card.name}。`;
