...
function fusionRecipe(s,materials){
  ...
  const name=affixes[0]?.name?`${affixes[0].name}${archetype.name}`.slice(0,8):archetype.name,source=cards.map(c=>c.name).join('＋'),text=[archetype.text,...affixes.map(a=>a.text)].join('，').slice(0,80),generation=1+Math.max(0,...cards.map(c=>c.lineage?.generation||0)),parents=cards.map(c=>c.name).slice(0,3);
  return {primary,name,kind:archetype.kind||primary.kind,race,element,mutation:archetype.mutation,passive:archetype.passive,effects,triggers,text,source,cost:count===3?5:4,affixes:affixes.map(a=>a.name),lineage:{generation,parents}};
}
...
export function fuseUnits(s,unitIds=[]){
  ...
  const hue=Math.floor(fusionRolls(s,[...unique,'hue'])()*300),card={id,name:recipe.name,kind:recipe.kind,race:recipe.race,element:recipe.element,mutation:recipe.mutation,passive:recipe.passive,fusion:{name:'融合',text:recipe.text,effects:recipe.effects,triggers:recipe.triggers},lineage:recipe.lineage,rarity:'rare',cost:recipe.cost,text:`融合异兽 · ${recipe.source}。${recipe.text}`,hue};
