...
export function restore(raw){
  try{
    ...
    if(!Array.isArray(s.mutants)||s.mutants.length>12||s.mutants.some(m=>!record(m)||typeof m.id!=='string'||!/^mut_[a-z0-9_]+$/.test(m.id)||typeof m.name!=='string'||m.name.length<1||m.name.length>8||!kinds.includes(m.kind)||!Object.hasOwn(RACES,m.race)||!['water','fire','spirit'].includes(m.element)||!Object.hasOwn(MUTATIONS,m.mutation)||!['common','rare'].includes(m.rarity)||!int(m.cost,3,5)||typeof m.text!=='string'||!int(m.hue,0,300)))return null;
