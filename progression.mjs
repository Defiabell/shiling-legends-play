...
export function forgeCreature(s,draft={}){
  ...
  const name=safeForgeName(draft.name,profile),source=/^https?:\/\//.test(link)?' · 已绑定模型链接':'';
  n.gold-=3;n.mutants=[...(n.mutants||[]),{id,name,kind:profile.kind,race:profile.race,element:profile.element,mutation:profile.mutation,passive:profile.passive,rarity:'rare',cost:3,text:`Meshy铸灵 · ${profile.name} · ${mutation.name}：${mutation.text}${source}`,hue:Math.floor(random(n)*300)}];
...
export function restore(raw){
  try{
    ...
    if(!Array.isArray(s.mutants)||s.mutants.length>12||s.mutants.some(m=>!record(m)||typeof m.id!=='string'||!/^mut_[a-z0-9_]+$/.test(m.id)||typeof m.name!=='string'||m.name.length<1||m.name.length>8||!kinds.includes(m.kind)||!Object.hasOwn(RACES,m.race)||!['water','fire','spirit'].includes(m.element)||!Object.hasOwn(MUTATIONS,m.mutation)||(m.passive!==undefined&&!Object.hasOwn(PASSIVES,m.passive))||!['common','rare'].includes(m.rarity)||!int(m.cost,3,5)||typeof m.text!=='string'||!int(m.hue,0,300)))return null;
