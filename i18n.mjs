...
const regexes=[
 [/第 (\d+) \/ (\d+) 战/g,'Battle $1 / $2'],[/([一二三四五六七八九十]+|\d+)代/g,'Gen $1'],[/第 (\d+) 战/g,'Battle $1'],[/第 (\d+) 场/g,'Battle $1'],[/(\d+) 金/g,'$1 Gold'],[/商店 (\d+)[星★]/g,'Shop Lv.$1'],[/商店(\d+)[星★]/g,'Shop Lv.$1'],[/经验 (\d+)(?:\/(\d+))?/g,(_,a,b)=>b?`XP ${a}/${b}`:`XP ${a}`],[/我方 (\d+) \/ 6/g,'Allies $1 / 6'],[/敌方 (\d+) 兽 · 最高 (\d+) 星/g,'Enemies $1 · Max $2★'],[/生命 \+(\d+)/g,'HP +$1'],[/伤害 \+(\d+)%/g,'Damage +$1%'],[/冷却 -(\d+)%/g,'CD -$1%'],[/生命 (\d+)/g,'HP $1'],[/攻 (\d+)/g,'ATK $1'],[/技能 ([0-9.]+)s/g,'Skill $1s'],[/冷却 ([0-9.]+)s/g,'CD $1s'],[/出售：(\d+)金/g,'Sell: $1 Gold'],[/购买 · 第 (\d+) 战孵化/g,'Buy · Hatch Battle $1'],[/补(\d+)金/g,'Pay $1 Gold'],[/KillSummon/g,'Kill Summon']
];
